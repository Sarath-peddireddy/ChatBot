...
def handle_general_query(query):
    ...
    elif 'what is law' in query:
        return "Law is a system of rules created and enforced through social or governmental institutions to regulate behavior. It is a means of social control and is designed to maintain order and protect individuals' rights."
    elif 'crime' in query:
...
def get_closest_section(query, _vectorizer, df, text_vectors):
    ...
    # Check for section number directly in the query using a comprehensive regex pattern
    section_match = re.search(r'\b(?:section|ipc section|section of ipc)\s*(\d+)\b', query, re.IGNORECASE)
    if section_match:
        section_number = section_match.group(1)
        # Filter the DataFrame for the requested section
        section_result = df[df['Section'].str.contains(section_number, case=False)]
        if not section_result.empty:
            result = section_result.iloc[0]
